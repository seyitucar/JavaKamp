...
public interface JobAdvertisementDao extends JpaRepository<JobAdvertisement, Integer>{
	...
	@Query("Select new com.seyitucar.hrmsSpring.entities.dtos.JobAdvertisementDto"
			+ "(id, jobTitle.titleName, employer.companyName, workingType.workingTypeName, workingPlace.workingPlaceName,"
			+ " numberOfOpenPosition, applicationDeadline, createdAt)"
			+ " From JobAdvertisement where isActive=:activityStatus")
	List<JobAdvertisementDto> getByActivity(@Param ("activityStatus") boolean activityStatus);

	@Query("Select new com.seyitucar.hrmsSpring.entities.dtos.JobAdvertisementDto"
			+ "(id, jobTitle.titleName, employer.companyName, workingType.workingTypeName, workingPlace.workingPlaceName,"
			+ " numberOfOpenPosition, applicationDeadline, createdAt)"
			+ " From JobAdvertisement where isActive=:activityStatus Order By applicationDeadline ")
	List<JobAdvertisementDto> getByActivityOrderByApplicationDeadline(@Param ("activityStatus") boolean activityStatus);
	
	@Query("Select new com.seyitucar.hrmsSpring.entities.dtos.JobAdvertisementDto"
			+ "(id, jobTitle.titleName, employer.companyName, workingType.workingTypeName, workingPlace.workingPlaceName,"
			+ " numberOfOpenPosition, applicationDeadline, createdAt)"
			+ " From JobAdvertisement where isActive=:activityStatus Order By applicationDeadline Desc ")
	List<JobAdvertisementDto> getByActivityOrderByApplicationDeadlineDesc(@Param ("activityStatus") boolean activityStatus);
	
	@Query("Select new com.seyitucar.hrmsSpring.entities.dtos.JobAdvertisementDto"
			+ "(j.id, jt.titleName, e.companyName, w.workingTypeName, wp.workingPlaceName , j.numberOfOpenPosition,"
			+ " j.applicationDeadline, j.createdAt)"
			+ " From JobAdvertisement j Inner join j.employer e"
			+ " Inner join j.workingType w Inner join j.workingPlace wp Inner join j.jobTitle jt where e.companyName=:companyName and j.isActive=:activityStatus")
